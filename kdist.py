...
def kdist(l, b, vin, near=True, r0=8.15e3, v0=2.47e2,
    ...
    if kinematic or not(dynamical):
        ...
        solarmotion_dec = (30 + 0 + 16.8/3600)
    ...
    elif np.isscalar(ind) and ind is True:
        neardist = fardist
    ...
    if np.any(vin > vtan):
        if not silent:
            warn("Velocity is greater than tangent velocity.\
                 Returning tangent distance.")
        if np.isscalar(dist):
            dist = rtan
        else:
    ...
    if verbose:
        print("radical: %f  null: %f  vin: %f  v: %f  vhelio: %f rgal: %f  \
              neardist: %f  fardist: %f".format(
              radical, null, vin, v, vhelio, rgal, neardist, fardist))
